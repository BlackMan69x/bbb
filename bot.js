...
bot.login("OTQxNjUzOTM5MjczNzMyMDk2.YgZFkg.D-lzahYNm7kKDzn21cyvKkjmPAM")
global.mongoose = require('mongoose')
mongoose.connect("mongodb+srv://DarkMan:darkman@darkman.xgzti.mongodb.net/myFirstDatabase?retryWrites=true&w=majority", { useNewUrlParser: true, useUnifiedTopology: true }).then(() => {
...
bot.on("ready", async () => {
/* let channel = bot.channels.cache.get("941637845368602625");
  channel.send(new Discord.MessageEmbed().setColor(Color).setTimestamp().setThumbnail(bot.user.displayAvatarURL()).setTitle("Whoami Status").addField("Prefix", "`s!`").addField("Status", "<:enabled:930487071536209940> Online").addField("Servers", `${bot.guilds.cache.size}`));*/
...
if (message.content.startsWith(prefix + "darkman")) {
        ...
        .setColor(Color).setTitle(`<:SectionBot:942681769629614090> is list for all commands`)
...
bot.on('ready', () => {
let channel = bot.channels.cache.get("936557957120950322");
...
bot.on("guildCreate", guild => {
  let channel = bot.channels.cache.get("937345385301704855");
