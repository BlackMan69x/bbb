...
module.exports = {
  ...
  run: async (bot, message, args, dev, data) => {
      ...
      if (args[1] === "add") {
        ...
        message.channel.send(new Discord.MessageEmbed().setColor(Color).setDescription(`<:true:923558420584464444> ${user.user.username} Added to whitelist`));
          } else {
        ...
        message.channel.send(new Discord.MessageEmbed().setColor(Color).setDescription(`<:false:923566366538235934> ${user.user.username} Removed in whitelist`));
        } else {
        message.channel.send(new Discord.MessageEmbed().setColor(Color).setDescription(`<:false:923566366538235934> ${user.user.username} Not in whitelist`));
        };
      } else if (!args[1]) {
