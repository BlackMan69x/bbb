...
async run(message,bot) {
 ...
 if (guild) {
  if (!message.content.toLowerCase().startsWith(guild.prefix.toLowerCase()) && !message.content.toLowerCase().startsWith("<@937063235612450827>")) return;
  ...
  const argsrP = await message.content
    ...
    .split(/ +/g);
  argsr.prefix = argsrP;
  } else if (message.content.toLowerCase().startsWith("<@941653939273732096>")) {
  const argsrM = await message.content
    .slice("<@941653939273732096>".length)
    .trim()
    .split(/ +/g);
  argsr.prefix = argsrM;
  };
...
//if(command.prime) {
 /* let data = await Prime.findOne({Guild: message.guild.id})
  ...
  let Ww = await Owner.findOne({ ownerCode: "937063235612450827" });
  data.ww = Ww;
